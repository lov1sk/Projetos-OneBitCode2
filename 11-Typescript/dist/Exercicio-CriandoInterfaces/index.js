 ...
 */
const getUsersAndSave = async (nameUser) => {
    const response = await (await fetch(`https://api.github.com/users/${nameUser}`)).json();
    const { name, bio, public_repos, repos_url } = response;
    const newUser = new GitHubUser({ name, bio, public_repos, repos_url });
    UsersDatabase.push(newUser);
};
const showRepos = async (name) => {
    ...
    response.forEach((res) => {
        ...
        console.log(`Os repositorios de "${name}":
        ${newRepo}`);
    });
};
const showUsers = () => {
